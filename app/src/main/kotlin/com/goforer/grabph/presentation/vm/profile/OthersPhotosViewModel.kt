...
@Inject
constructor(val interactor: OthersPhotosRepository): BaseViewModel() {
    @VisibleForTesting
    private val liveData by lazy { MutableLiveData<PhotogQuery>() }
    ...
    init {
        userProfile = Transformations.switchMap(liveData) { query ->
            ...
            liveData(context = viewModelScope.coroutineContext + Dispatchers.IO) {
                emitSource(interactor.load(this@OthersPhotosViewModel, query?.userID!!, query.pages, loadType, boundType, calledFrom))
            }
        }
    }
