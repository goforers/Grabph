...
class TopPortionQuestViewModel
@Inject
constructor(private val interactor: TopPortionQuestRepository): BaseViewModel() {
    @VisibleForTesting
    ...
    init {
        quest = Transformations.switchMap(liveData) { query ->
            ...
            liveData(context = viewModelScope.coroutineContext + Dispatchers.IO) {
                emitSource(interactor.load(this@TopPortionQuestViewModel, query!!, -1, loadType, boundType, calledFrom))
            }
        }
    }

    @MockData
    internal fun loadTopPortionQuest(): LiveData<TopPortionQuest>? = liveData(context = viewModelScope.coroutineContext + Dispatchers.IO) { emitSource(interactor.loadTopPortionQuest()) }
    ...
    @MockData
    internal fun setTopPortionQuest(topPortionQuest: TopPortionQuest) {
        viewModelScope.launch {
            interactor.setTopPortionQuest(topPortionQuest)
        }
    }
}