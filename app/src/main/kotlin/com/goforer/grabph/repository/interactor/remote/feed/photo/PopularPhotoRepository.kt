...
import com.goforer.grabph.repository.interactor.remote.paging.boundarycallback.PageListPopularPhotoBoundaryCallback
...
@Singleton
class PopularPhotoRepository
@Inject
constructor(private val dao: PhotoDao): Repository() {
    companion object {
        const val METHOD = "flickr.photos.getPopular"
    }
            ...
            override suspend fun loadFromCache(isLatest: Boolean, itemCount: Int,
                ...
                return withContext(Dispatchers.IO) {
                    LivePagedListBuilder(dao.getPhotos(), /* PageList Config */ config)
                            .setBoundaryCallback(PageListPopularPhotoBoundaryCallback<Photo>(
                                    viewModel as PopularPhotoViewModel, query1, pages, calledFrom)).build()
                }
            }

            override suspend fun loadFromNetwork() = searpService.getPopularPhotos(KEY, query1, METHOD, FORMAT_JSON, query2, PER_PAGE, INDEX)
