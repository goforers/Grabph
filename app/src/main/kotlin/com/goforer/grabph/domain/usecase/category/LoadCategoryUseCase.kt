...
package com.goforer.grabph.domain.usecase.category
...
import com.goforer.grabph.data.repository.remote.category.CategoryRepository
...
@Inject
constructor(private val repository: CategoryRepository):  BaseUseCase<Parameters, Resource>() {
    ...
    @MockData
    internal fun loadCategories(): LiveData<List<Category>>? = repository.loadCategories()

    @MockData
    internal suspend fun setCategories(categories: List<Category>) = repository.setCategory(categories as MutableList)

    internal suspend fun deleteCategory() = repository.deleteCategory()
}