...
@Singleton
class LoadFavoritePhotoUseCase
@Inject
constructor(private val repository: FavoritePhotoRepository):  BaseUseCase<Parameters, Resource>() {
    ...
    override fun execute(viewModelScope: CoroutineScope, parameters: Parameters): LiveData<Resource> {
        ...
        return Transformations.switchMap(liveData) { query ->
            ...
            liveData(context = viewModelScope.coroutineContext + Dispatchers.IO) {
                emitSource(repository.load(liveData, Parameters(query.query, query.pages, liveData.value?.loadType!!, liveData.value?.boundType!!)))
            }
        }
    }
    ...
    internal suspend fun removePhotos() = repository.removePhotos()

    internal fun deleteByPhotoId(id: String) = repository.deleteByPhotoId(id)

    internal suspend fun update(photo: Photo) = repository.update(photo)
}